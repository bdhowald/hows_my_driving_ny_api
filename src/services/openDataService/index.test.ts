...
describe('OpenDataService', () => {
    ...
    describe.each([true, false])(
      ...
      (useSocrataSodaV3Api) => {
        describe('querying various open data tables', () => {
          ...
          const axiosFunctionToMock = useSocrataSodaV3Api
            ? axios.post
            : axios.get
          ...
          it('should use the most recent identified medallion plate', async () => {
            ...
            const medallionDatabaseQueryParams =
              getMedallionDatabaseQueryParams(identifiedMedallionPlate)
          ...
          describe('handling open data errors', () => {
            ...
            it('should log and rethrow a non-axios error', async () => {
              ...
              ;(axiosFunctionToMock as jest.Mock).mockRejectedValue(
                nonAxiosError
              )
  ...
  describe('getV2EndpointForDatabaseFromV3Endpoint', () => {
    it('should return the v2 version of a v3 endpoint', () => {
      const v2Endpoint = 'https://data.cityofnewyork.us/resource/jt7v-77mi.json'
      const v3Endpoint =
        'https://data.cityofnewyork.us/api/v3/views/jt7v-77mi/query.json'

      expect(
        OpenDataService.getV2EndpointForDatabaseFromV3Endpoint(v3Endpoint)
      ).toBe(v2Endpoint)
    })

    it('should return a v2 endpoint if supplied', () => {
      const v2Endpoint = 'https://data.cityofnewyork.us/resource/jt7v-77mi.json'

      expect(
        OpenDataService.getV2EndpointForDatabaseFromV3Endpoint(v2Endpoint)
      ).toBe(v2Endpoint)
    })

    it('should return a quasi-v3 endpoint as-is if only the v3 pathname prefix is found', () => {
      const quasiV3Endpoint =
        'https://data.cityofnewyork.us/api/v3/views/jt7v-77mi.json'

      expect(
        OpenDataService.getV2EndpointForDatabaseFromV3Endpoint(quasiV3Endpoint)
      ).toBe(quasiV3Endpoint)
    })

    it('should return a quasi-v3 endpoint as-is if only the v3 pathname suffix is found', () => {
      const quasiV3Endpoint =
        'https://data.cityofnewyork.us/resources/jt7v-77mi/query.json'
    ...
    it('should return any string other than a v3 endpoint as-is', () => {
      const someOtherString =
        'should return any string other than a v3 endpoint as-is'

      expect(
        OpenDataService.getV2EndpointForDatabaseFromV3Endpoint(someOtherString)
      ).toBe(someOtherString)
    })
  })
