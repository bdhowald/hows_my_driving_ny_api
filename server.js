...
normalizeViolations = (violations) => {
  ...
  violations.forEach((violation) => {
    ...
    const fiscalYearDescriptionKey = violation.violation_code || violation.violation_description

    let fyHumanizedDescription = undefined

    if (fiscalYearDescriptionKey) {
      violationCodeDefinition = fyHumanizedNames[fiscalYearDescriptionKey]
      if (violationCodeDefinition) {
        if (typeof(violationCodeDefinition) === 'object' && violation.issue_date) {
          // Some violation codes have applied to multiple violations.
          violationCodeDefinition.forEach((possibleDescription) => {
            if (Date.parse(possibleDescription.startDate) < Date.parse(violation.issue_date)) {
              fyHumanizedDescription = possibleDescription.description
            }
          })
        } else {
          fyHumanizedDescription = violationCodeDefinition
        }
      }
    }
    ...
    const newViolation = {
      ...
      humanized_description             : fyHumanizedDescription || opacvHumanizedNames[violation.violation] || null,
      ...
      violation_code                    : (violation.violation_code || namesToCodes[fyHumanizedDescription || opacvHumanizedNames[violation.violation]]) || null,
